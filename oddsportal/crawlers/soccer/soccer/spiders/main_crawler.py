...
links = open('/home/desprit/Documents/Python/examples/scrapy/soccer/soccer/spiders/links_03.txt').readlines()
...
class CakeSpider(CrawlSpider):
    ...
    '''
   "http://www.oddsportal.com/soccer/australia/",
   "http://www.oddsportal.com/soccer/armenia/",
   "http://www.oddsportal.com/soccer/austria/",
   "http://www.oddsportal.com/soccer/bahrain/",
   "http://www.oddsportal.com/soccer/belarus/",
   "http://www.oddsportal.com/soccer/belgium/",
   "http://www.oddsportal.com/soccer/bolivia/",
   "http://www.oddsportal.com/soccer/azerbaijan/"
    '''

    '''
    def parse(self, response):
        hxs    = HtmlXPathSelector(response)
        groups = hxs.select("//table[@id='country-tournaments-table']//tr[not(@class='dark center')]//a").extract()
        league = hxs.select("//table[@id='country-tournaments-table']//tr[@class='dark center']//a[@class='bfl']/text()").extract()[0]
        
        for group in groups:
            group_url  = re.findall(re.compile('href=\"(.+?)\"'),  group)[0]
            group_name = re.findall(re.compile('>(.+?)</a>'), group)[0]

            new_url = urljoin_rfc(get_base_url(response), group_url)
            new_url = new_url + 'results'
            yield Request((new_url), meta = {'group':  group_name,
                                             'league': league}, callback=self.parse_group)
    '''
